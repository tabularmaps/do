...
(function () {
  ...
  function codes() {
    if (!codesPromise) codesPromise = fetch('./data/jma-warning-codes.json').then((r) => r.json());
    return codesPromise;
  }
  ...
  async function fetchValues() {
    ...
    const reports = await Promise.all(OFFICES.map(([code]) =>
      fetch(BASE + code + '.json', { cache: 'no-store' }).then((r) => r.json()).catch(() => null)));
    const level = {}, names = {};
    ...
    return {
      ...
      scale: { type: 'ordinal', labels: table.levels }
    };
  }

  window.TABULARMAPS_JMA_WARNINGS = {
    key: 'jma-warnings', name: '気象警報・注意報 (気象庁)', refreshMs: 5 * 60 * 1000, fetchValues
  };
})();
